...
* **Display del descuento (`web/orders/show`):** el descuento y el redondeo se muestran como **dos líneas separadas** para no confundirlos. `Order#discount_amount` es el descuento **nominal** (suma de `qty × unit × discount_percent/100` por ítem), **no** `original − total` — derivarlo del total mezclaría el ceil con el descuento (bug histórico: con `total_amount` ya redondeado, `original − total` daba 2.400 en vez del 10% real de 2.450). `Order#rounding_amount` = `total_amount − (original_total_amount − discount_amount)` expone el cargo de ceil-a-100 por separado (0 cuando no hubo redondeo, ej. descuentos por ítem de crédito). Reconcilia: `Subtotal − Descuento + Redondeo = Total`. `total_amount` se mantiene como el valor redondeado canónico (las `payment_allocations` cierran exacto).
