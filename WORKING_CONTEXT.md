...
* **Stock is not modified when selling today** — `Sales::CreateOrder` does not create a `StockMovement`. The availability validation runs **only on `source: 'live'`**; the `web/orders/new` form submits **`source: from_paper` by default**, so in practice **stock is not validated when selling** (products with no stock and arbitrary quantities can be added; see Key constraints). **`Sales::CancelOrder` does NOT restock** — `reverse_stock_movements` is defined but its call site is commented out (`app/services/sales/cancel_order.rb`). See "Dormant stock code" below.
* Immediate discount: caja applies it at collection time via `Payments::CollectSaleNote`. Cap `0 / 5 / 10` (global), distributed across all `order_items`. **Rule:** only allowed if the whole amount is paid in cash (validated in the backend and enforced in Stimulus). **Nearest-hundred rounding (cash with discount):** when there is a discount (>0), the cash to collect = `original_total × (1 − desc/100)` rounded to the **nearest** multiple of 100 via `Payments::CashRounding.round_to_nearest_hundred` — a remainder of exactly 50 rounds **down** (`BigDecimal :half_down`; e.g. `710.775 × 0,90 = 639.697,5 → 639.700`, but `22.050 → 22.000`). `apply_discount!` sets `total_amount = effective_total` (the canonical rounded value); `order_item.discount_percent` remains as display metadata. Without discount: exact, no rounding. The front end uses the shared helper `app/javascript/helpers/cash_rounding.js` (`roundToNearestHundred`), imported by the `sale_note_payment`, `payment_allocation`, `on_account_payment` and `order_form` controllers. **Ruby and JS are two independent implementations of one rule** — they are kept in sync by a system spec (see `AGENTS.md` → Testing Rules → Rule A).
* **Discount display (`web/orders/show`):** the discount and the rounding are shown as **two separate lines** so they are not confused. `Order#discount_amount` is the **nominal** discount (sum of `qty × unit × discount_percent/100` per item), **not** `original − total` — deriving it from the total would mix the rounding with the discount (historical bug: with `total_amount` already rounded, `original − total` gave 2.400 instead of the real 10% of 2.450). `Order#rounding_amount` = `total_amount − (original_total_amount − discount_amount)` exposes the nearest-hundred adjustment separately (0 when there was no rounding, e.g. per-item credit discounts). It reconciles: `Subtotal − Discount + Rounding = Total`. `total_amount` remains the canonical rounded value (the `payment_allocations` close exactly).
...
* **Repeatable partial collection** via `Payments::CollectOnAccount` (sibling of `CollectSaleNote`). It receives `order:, amount_to_settle:, discount_percent:, tenders:`. `amount_to_settle` = how much of the balance this visit cancels (hard guard `≤ outstanding_balance`). Per-event discount `0/5/10`, **cash only**. **Nearest-hundred rounding (cash with discount):** the cash to collect (`cash_to_collect`) = `amount_to_settle × (1 − desc/100)` rounded to the **nearest** multiple of 100 (`Payments::CashRounding.round_to_nearest_hundred`, ties round down); without discount, exact. `apply_discount!` **lowers `total_amount`** by the **effective discount** = `amount_to_settle − cash_to_collect` (NOT by the nominal discount), so the balance closes exactly against the rounded allocation (`original_total_amount` untouched). The `PaymentAllocation` records the rounded cash. The controller (`Web::PaymentsOnAccount::PaymentsController`) builds the tender with the **same** helper, so service and controller cannot drift. It calls `refresh_status_from_balance!` at the end. **For spec amounts use a nominal discount ≥ 100** (keeps the rounding from pushing `cash_to_collect` back above `amount_to_settle`).
...
* **Inventory:** `Inventory::MarkDelivered`
* **Invoices:** `Invoices::CreateSimpleInvoice`, `Invoices::MarkAsPaid`, `Invoices::ProcessPayment`
* **Payments:** `Payments::AllocatePayment`, `Payments::CollectSaleNote`, `Payments::CollectOnAccount`, `Payments::CashRounding`
...
### Dormant stock code (present, NOT wired — reserved for the upcoming stock feature)

Stock is currently **inert**: `current_stock` only moves via the `Inventory::SyncFromCsv` rake task. The following exist but no live code path reaches them. **Do not delete them, and do not write specs for them yet** — they are reserved for the incoming stock-management feature.

* **`Inventory::AdjustStock`** — the only sanctioned path for mutating stock. `Web::Products::StockMovementsController` and `app/views/web/products/stock_movements/new.html.haml` exist, but **no view links to them** (reachable only by typing the URL).
* **`Sales::CancelOrder#reverse_stock_movements`** — defined, call site commented out. The matching `xit` in `spec/services/sales/cancel_order_spec.rb` is therefore **correct**, not a coverage gap.
* **`Inventory::CalculateStockLevel`** — zero callers.
* **`Purchasing::CreatePurchase` / `CancelPurchase`** — no controller.

**When that feature wires these up:** `Inventory::AdjustStock` needs unit + request specs on day one (it guards the critical stock rule in `CLAUDE.md`), and the `cancel_order_spec.rb` `xit` must be re-enabled.
