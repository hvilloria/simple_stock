...
> **Superseded.** This is the first design of the cash book, written before the
> module was built. What was actually built — and every decision taken during the
> build — lives in `docs/CASH_MODULE_DESIGN.md`. Kept for the history of how the
> module was first conceived; do not read it as current behaviour.
