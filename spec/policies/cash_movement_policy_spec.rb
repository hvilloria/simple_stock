...
RSpec.describe CashMovementPolicy do
  ...
  describe "#index? and #create?" do
    it "is the admin's alone: the cashier's access to the module is deferred" do
      expect(described_class.new(admin, CashMovement).index?).to be(true)
      expect(described_class.new(cashier, CashMovement).index?).to be(false)
      ...
      expect(described_class.new(cashier, CashMovement).create?).to be(false)
  ...
  describe "#update? and #destroy?" do
    ...
    it "permits the admin on an open movement, and forbids the cashier and the seller" do
      expect(described_class.new(admin, open_movement).update?).to be(true)
      expect(described_class.new(cashier, open_movement).update?).to be(false)
      ...
      expect(described_class.new(cashier, open_movement).destroy?).to be(false)
  ...
  describe "Scope" do
    it "gives the admin every movement" do
      movement = create(:cash_movement)

      expect(described_class::Scope.new(admin, CashMovement).resolve).to include(movement)
    end

    it "gives the cashier and the seller nothing" do
      create(:cash_movement)

      expect(described_class::Scope.new(cashier, CashMovement).resolve).to be_empty
